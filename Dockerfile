...
RUN rm apollo.tar.gz && test -e apollo-${VERSION} && \
...
RUN bash apollo-${VERSION}/scripts/build.sh
...
RUN unzip apollo-${VERSION}/apollo-portal/target/apollo-portal-${VERSION}-github.zip -d /apollo-portal && \
    sed -i -e "s/^SERVER_PORT=.*$/SERVER_PORT=$PORTAL_PORT/" /apollo-portal/scripts/startup.sh

RUN unzip apollo-${VERSION}/apollo-adminservice/target/apollo-adminservice-${VERSION}-github.zip -d /apollo-admin/dev && \
...
RUN unzip apollo-${VERSION}/apollo-configservice/target/apollo-configservice-${VERSION}-github.zip -d /apollo-config/dev && \
...
RUN rm -rf apollo-${VERSION} && rm -rf /root/.m2
